...
5. **Tests gate the build (R4, shipped).** `npm test` runs a Vitest + RTL unit suite over the extracted decision math in [`src/lib/scoring.ts`](src/lib/scoring.ts) (RICE/WSJF/ROI/opportunity/maturity) plus the portfolio rollups and formatters; `npm run test:e2e` runs a Playwright smoke suite over the seeded build — every primary route mounts and one prioritization workflow re-ranks. Extracting the scoring math into one tested module also removed the seeded/live duplication (both copies now import it), so the tests cover the *shipped* code path, not a copy.
...
- **R4 — Tests. ✅ SHIPPED 2026-08-02.** The decision math was extracted into one shared [`src/lib/scoring.ts`](src/lib/scoring.ts) (RICE/WSJF/Value-vs-Effort/Opportunity, weighted opportunity rollup, the 24-month ROI model, maturity average + gaps) that both the seeded and live copies now import — killing the prior duplication so tests cover the shipped path. **43 Vitest + RTL unit tests** cover that module plus the portfolio rollups and formatters; a **Playwright smoke suite** drives the seeded build — the nav rail and all 13 routes mount (proving the R3 lazy chunks load) and switching prioritization frameworks re-ranks without error. `npm test` / `npm run test:e2e`.
- **R9 — Refresh cadence. ✅ SHIPPED 2026-08-02.** Scheduled GitHub Actions in the *source* repos keep the live snapshots current. **FI** (`refresh-snapshot.yml`, daily) re-pulls the real StatCan/CMHC/BoC indicators, regenerates `web/studio-snapshot.json`, commits any change and **deploys Pages inline** — closing the loop with no secrets (the adapters are keyless), since the Studio fetches that snapshot straight from Pages. **RAG** is split by design: the operational half of `/snapshot` (latency, grounded rate, cost/query, volume) is **already live** from the observability store on every request, so only the judged `eval/summary.json` needs a cadence — `refresh-eval.yml` (weekly) regenerates it via `eval:summary` (a generator that also computes knowledge freshness live), guarded to skip cleanly without `DATABASE_URL` + `JUDGE_API_KEY` (so a fork runs at $0); because the Cloud Run container reads the committed summary baked at deploy, that refresh reaches the live endpoint on the next service deploy.
- **R7 — Board-ready export. ✅ SHIPPED 2026-08-06.** A one-click **Export PDF** on the Executive Dashboard and Cross-Product scorecard downloads a board-ready report — cover, KPI grid, executive summary, top opportunities, and the products table — generated **entirely client-side ($0, nothing leaves the browser)**. jsPDF + html-to-image are dynamically imported (their own lazy chunk, so first load is untouched, per R3) and capture a dedicated fixed-width [`BoardReport`](src/live/ui/report/BoardReport.tsx) view. The rollup math was extracted to a shared, unit-tested [`src/live/report/rollups.ts`](src/live/report/rollups.ts) that the dashboard and the report both use, so the PDF matches the screen exactly (11 Vitest tests + a Playwright download test).
- **R8 — Real observability across all products. ✅ SHIPPED 2026-08-17.** A new **Live Observability** view unifies reliability, cost and data freshness across every product into one honest picture: universal signals (reachability + measured endpoint latency) show for all; RAG's full runtime metrics (p50/p95, cost/query, volume, grounded/error rate) appear where the source exposes them; and **data freshness + lineage** is surfaced for the static sources (FI's live provenance, each snapshot's last-updated). Everything else reads an explicit "Not reported," never seeded. The Executive Dashboard gains a portfolio **data-freshness** signal (the stalest source's age). The derivation math is extracted to a unit-tested [`src/live/report/observability.ts`](src/live/report/observability.ts) (later extended per-adapter — see R8+ below — now **14 Vitest tests** + a Playwright route test). $0, client-side, no source redeploys.
- **R8+ — Per-adapter observability + de-biased rollup. ✅ SHIPPED 2026-08-18.** The Live Observability cards were RAG-shaped for every product, so non-RAG products showed a wall of "Not reported" (grounded rate, cost/query, p95 — metrics they don't have). The cards are now **adapter-aware**, showing each product's *honest applicable* signals: the **Diagnostic** (readiness) surfaces readiness score, assessments, completion and 7/30-day activity + uptime (via a new `observability` block on its snapshot); the **Financial** agent surfaces data sources, indicators, **oldest input** (the **stalest** series' ref period, with reporting-lag hint — a conservative staleness flag for a mixed-cadence data agent), history depth and decision traces (most derived from the snapshot it already sends; a small [FI-repo change](docs/R8b-financial-repo-prompt.md) makes history-depth/pull-time authoritative). The **top KPI row was also de-biased** — it led with "Avg latency p95," which was just RAG's number posing as a portfolio average; it now leads with the three signals *every* product shares (reachability, endpoint latency, data freshness) and keeps inference cost as an honest Σ-where-metered tile. No fabricated telemetry anywhere. Client-side; the source-repo prompts ([Diagnostic](docs/R8-diagnostic-repo-prompt.md), [FI](docs/R8b-financial-repo-prompt.md)) are additive and optional.
- **R6a — Auth (magic-link sign-in). ✅ SHIPPED 2026-08-17.** Optional, progressive **passwordless** sign-in on the existing Worker + Neon ($0): enter an email → the Worker emails a one-time link (Resend) → a short-TTL **HMAC-signed JWT** (localStorage, sent as `Bearer`) upgrades the device-local **"Acting as"** name to a **verified identity** that governance actions attribute to. Anonymous browsing still works, and the auth endpoints return 501 (the SPA degrades silently) until configured — one-time setup (Neon migration + secrets + Worker deploy) in [`docs/AUTH.md`](docs/AUTH.md). Only a SHA-256 hash of each single-use token is stored. 8 Vitest tests on the client JWT helpers.
- **R6b — Roles / RBAC. ✅ SHIPPED 2026-08-18.** A role per user (**viewer < contributor < approver < admin**), enforced **server-side**: advancing a governance stage (`POST /api/workflow`) now requires a signed-in **approver/admin** (anonymous → 401, wrong role → 403), and the audit records the *verified* user, not a client-supplied name. Admins manage roles in a new **Users & Roles** view (`GET /api/users` / `POST /api/users/role`, admin-only); admins are bootstrapped via an `ADMIN_EMAILS` var. The client mirrors the rules as affordances (approve buttons disable with a clear "Approver required" hint) but the Worker is authoritative. Pure role logic is unit-tested (5 tests); setup in [`docs/AUTH.md`](docs/AUTH.md).
- **R6c — Per-org multi-tenant. ✅ SHIPPED 2026-08-18.** Every user belongs to an **org**, and all Studio data (`registrations`, `assessments`, `workflow_stages`, `audit_events`, `studio_entities`) is scoped by `org_id`, **enforced server-side** — the Worker derives the tenant from the verified session (never the client) and every read filters / every write stamps it, with `org_id` folded into the composite keys so orgs can't collide. `ADMIN_EMAILS` now grants **platform super-admin**, who get an **Organizations** console to create (template-seeded), rename, and suspend orgs; members join via an org admin's **Invite**. A real cross-org **isolation test** (`npm run test:isolation`) drives the Worker with two orgs and proves org B can't see or delete org A's data. Phased R6c-a (foundation) → R6c-b (enforcement) → R6c-c (management); full plan in [`docs/R6C-PLAN.md`](docs/R6C-PLAN.md). ($0 — still static Pages + one Worker + Neon.)
...
> **The entire roadmap is shipped** — Near-term (R3, R4, R9) and Stretch (R7, R8, R6a/b/c) are all done; **only R14d stays deliberately deferred**. The executive views also export an editable **PPTX deck** alongside the PDF, and a super-admin can **view any org read-only** from the Organizations console. The optional observability follow-ons are also done (see **R8+** above): both the **Diagnostic** (readiness) and the **Financial** agent now render adapter-appropriate tiles instead of RAG-shaped "Not reported," and the Live Observability top row was de-biased to lead with portfolio-universal signals — no fabricated telemetry, since these tools record none.
