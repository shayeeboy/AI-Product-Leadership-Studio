...
**Stretch**
- **R6c — Per-org multi-tenant (remaining).** Building on the R6a/b identity + roles: an `org_id` scoping `studio_entities`, `registrations`, `workflow_stages` and `audit_events`, derived from the session and enforced in the Worker so orgs can't see each other's data. Scoped in [`docs/STRETCH-PLAN.md`](docs/STRETCH-PLAN.md).
...
> **R7, R8, R6a, and R6b shipped; only R6c remains** (per-org multi-tenant), scoped in [`docs/STRETCH-PLAN.md`](docs/STRETCH-PLAN.md); R14d stays deferred.
