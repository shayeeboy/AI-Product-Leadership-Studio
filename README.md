...
**Stretch**
- **R6b/c — Roles + multi-tenant (remaining).** Building on the R6a verified identity: **R6b** role-based approvals (server-enforced who-may-approve-which-stage: viewer/contributor/approver/admin) and **R6c** per-org multi-tenant data isolation (an `org_id` scoping `studio_entities`, `registrations`, `workflow_stages`, `audit_events`, enforced in the Worker). Scoped in [`docs/STRETCH-PLAN.md`](docs/STRETCH-PLAN.md).
...
> **R7, R8, and R6a shipped; only R6b/c remain** (roles + multi-tenant), scoped in [`docs/STRETCH-PLAN.md`](docs/STRETCH-PLAN.md); R14d stays deferred.
