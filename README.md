...
**Phase 3 — full module parity, live** — bring every seeded module into the live copy on **Studio-managed data** (registration metadata + user-entered data persisted to R1 Neon), no further source-app changes. The **R10 foundation is shipped** (above); R11–R13 are the module migrations. Full plan: [`docs/PHASE-3-PLAN.md`](docs/PHASE-3-PLAN.md).
- **R11 — Decision modules live (Phase 3A).** ← *next.* Opportunity Assessment, Build vs Buy, Cost Analyzer, ROI Simulator, Investment Prioritization, Maturity — wired to persistence; anchored on real RAG cost/query + Diagnostic readiness where they slot in.
